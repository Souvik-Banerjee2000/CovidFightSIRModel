...
class SIR:
    def __init__(self, eons=2000, Susceptible=800, Infected=1200, Resistant=0, rateSI=0.05, rateIR=0.01):
        self.eons = eons
    ...
    def plot(self):
        ...
        plt.plot(self.results['Time'],
                 self.results['Resistant'], color='green')
        plt.xlabel('Time')
        plt.ylabel('Population')
        plt.legend(['Susceptible', 'Infected', 'Resistant'], prop={
                   'size': 10}, loc='lower center', bbox_to_anchor=(0.5, 1.02), ncol=3, fancybox=True, shadow=True)
